...
export default function PhysicsPanel({run,time,onRun}:{run?:PhysicsRun;time:number;onRun:(run:PhysicsRun)=>void}){
  ...
  async function start(){
    ...
    try{const r=await fetch('/api/physics/run',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({duration:seconds,drive,turn,silenced,feedback,vision,visionControl,targetY,odour,odourControl})});const s=await r.json();if(!r.ok)throw new Error(s.error);}
