...
export default function CircuitView({ data }: { data: CircuitInspection }) {
    ...
    <>
      ...
      >
        ...
        <p>
          ...
          {step.tokenId}. Embeddings enter the marked input cells at both
          updates. State retention and biases also contribute.
        </p>
            ...
            {step.states.flatMap((values, k) =>
              values.map((v, i) => (
                <g key={`${k}-${i}`}>
                  ...
                  >
                    <title>
                      Cell {data.cells[i].id}; activation {v.toFixed(6)};
                      injected embedding {step.input[i].toFixed(6)}; retention{" "}
        ...
        <p>
          {data.selection} Hidden cells and their connections still contribute
          to every update. Hover a node or line for its value. Column order is
