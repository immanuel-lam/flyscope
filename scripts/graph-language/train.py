...
from model import GraphLanguageModel,AttentionControl,ROOT,wiring,input_coverage
DATA=ROOT/'data/graph-language'

def main():
    p=argparse.ArgumentParser();p.add_argument('--steps',type=int,default=10000);p.add_argument('--batch',type=int,default=8);p.add_argument('--width',type=int,default=256);p.add_argument('--layers',type=int,default=4);p.add_argument('--run',default='run-2');p.add_argument('--init',default=None);p.add_argument('--architecture',choices=['graph','control'],default='graph');a=p.parse_args()
    mx.random.seed(127);rng=np.random.default_rng(127);model=(GraphLanguageModel if a.architecture=='graph' else AttentionControl)(width=a.width,layers=a.layers);optimizer=optim.AdamW(learning_rate=.0003,weight_decay=.01)
    initial_sha=None
    if a.init:
    ...
    coverage=input_coverage(inputs,outputs,mask,a.layers)
    if a.architecture=='control':
        inputs=outputs=slots=np.arange(128);mask=np.tril(np.ones((128,128),bool));source=np.zeros((128,128),bool);coverage=mask
    ...
    np.savez_compressed(out/'wiring.npz',inputs=inputs,outputs=outputs,slots=slots,mask=mask,sourceMask=source)
    print(json.dumps({'initialValidationLoss':initial,'parameters':parameters,'sourceEdgesUsed':int(np.count_nonzero(mask&source&~np.eye(len(mask),dtype=bool)))}),flush=True)
    for step in range(1,a.steps+1):
        ...
        if step==1 or step%200==0 or step==a.steps:
            ...
            (out/'training.json').write_text(json.dumps({'arguments':vars(a),'seed':127,'initialCheckpointSha256':initial_sha,'architecture':a.architecture,'wiringVersion':2 if a.architecture=='graph' else None,'generationContextSlotsReachable':int(coverage[-1].sum()),'auxiliaryCompleteContextSlots':np.flatnonzero(np.all(coverage|~np.tril(np.ones((128,128),bool)),axis=1)).tolist(),'parameters':parameters,'initialValidationLoss':initial,'bestValidationLoss':best,'history':history,'assumptions':('Ordinary causal token-attention diagnostic. No biological source wiring; must not supply FlyGPT replies.' if a.architecture=='control' else '128 token slots mapped to distinct input/readout source cells. Causal source-edge attention, local self-retention and local feed-forward operations. Engineered feature vectors, not biological compartments. No external LM or logits bypass.')},indent=2))
