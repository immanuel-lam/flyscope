...
def main():
    mx.random.seed(89);rng=np.random.default_rng(89);model=MemoryCircuit();run=DATA/'run-1';model.load_weights(str(run/'dialogue.safetensors'))
    ...
    def loss(m,x,y,mask):
        ...
        return mx.sum(nn.losses.cross_entropy(m(x),y,reduction='none')*weight)/mx.maximum(weight.sum(),1)
    ...
    initial=evaluate();best=initial;model.save_weights(str(run/'short.safetensors'));history=[];optimizer=optim.AdamW(learning_rate=.0005,weight_decay=.001);grad=nn.value_and_grad(model,loss);start=time.perf_counter()
    ...
    for step in range(1,2001):
        ...
        if step%200==0 or step==1:
            ...
            if val<best:best=val;model.save_weights(str(run/'short.safetensors'))
            (run/'short-training.json').write_text(json.dumps({'seed':89,'initialValidationLoss':initial,'bestValidationLoss':best,'history':history,'objective':'Complete short source contexts, first 16 reply tokens weighted 2x. No authored answers. Validation-selected checkpoint.','source':str(DATA/'short-context/source.json')},indent=2))
