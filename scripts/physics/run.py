...
def run(duration=2,drive=1,turn=0,silenced=False,feedback=True,output=None,vision=False,vision_control=True,target=(12,4)):
    ...
    for step in range(steps+1):
        ...
        if step%neural_stride==0:
            ...
            signal=brain.step(drive,float(np.clip(turn+(visual_turn if vision and vision_control else 0),-1,1)),contacts,silenced,feedback)
    ...
    result={'schemaVersion':1,'rig':'neuromechfly-2.1.0','datasetId':brain.catalog['id'],'datasetVersion':brain.catalog['version'],'times':times,'frames':frames,'geometry':geometry,'bodyNames':[b.name for b in fly.get_bodysegs_order()],'activity':activity,
      'parameters':{'duration':duration,'drive':drive,'turn':turn,'silenced':silenced,'feedback':feedback,'seed':0,'vision':vision,'visionControl':vision_control,'target':list(target)},
