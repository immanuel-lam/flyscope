...
def run(duration=2,drive=1,turn=0,silenced=False,feedback=True,output=None,vision=False,vision_control=True,target=(12,4),odour=False,odour_control=True,obstacles=False,avoidance=True,layout="training",memory=False,cue_side="left",reset_memory=False):
    ...
    result={'schemaVersion':1,'rig':'neuromechfly-2.1.0','datasetId':brain.catalog['id'],'datasetVersion':brain.catalog['version'],'times':times,'frames':frames,'geometry':geometry,'bodyNames':[b.name for b in fly.get_bodysegs_order()],'activity':activity,
      'parameters':{'duration':duration,'drive':drive,'turn':turn,'silenced':silenced,'feedback':feedback,'seed':0,'vision':vision,'visionControl':vision_control,'target':list(target),'odour':odour,'odourControl':odour_control,'obstacles':obstacles,'avoidance':avoidance,'layout':layout,'memory':memory,'cueSide':cue_side,'resetMemory':reset_memory},
