  ...
  function install(server: ViteDevServer | PreviewServer) {
    ...
    server.middlewares.use('/api/physics',async(req,res,next)=>{
      ...
      try{
        ...
        for(const key of ['odour','odourControl','obstacles','avoidance','memory','resetMemory','backflip'])if(p[key]!==undefined&&typeof p[key]!=='boolean')throw new Error('Invalid odour option');
        ...
        if(p.cueSide!==undefined&&!['left','right'].includes(p.cueSide))throw new Error('Invalid memory cue');
        if(p.memory&&p.duration<2)throw new Error('Memory trials need at least 2 seconds');
