  ...
  function install(server: ViteDevServer | PreviewServer) {
    ...
    server.middlewares.use('/api/physics',async(req,res,next)=>{
      ...
      try{
        ...
        for(const key of ['odour','odourControl'])if(p[key]!==undefined&&typeof p[key]!=='boolean')throw new Error('Invalid odour option');
        ...
        child=spawn(python,[resolve(root,'scripts/physics/run.py'),'--duration',String(p.duration),'--drive',String(p.drive),'--turn',String(p.turn),...(p.silenced?['--silenced']:[]),...(!p.feedback?['--no-feedback']:[]),...(p.vision?['--vision']:[]),...((p.vision||p.odour)?['--target-y',String(p.targetY??4)]:[]),...(p.odour?['--odour']:[]),...(p.odourControl===false?['--no-odour-control']:[]),...(p.visionControl===false?['--no-vision-control']:[])],{cwd:root,stdio:['ignore','pipe','pipe']});
        let pending='',stderr='';
