...
## final-readout run outcome

`adapt-readout-1` finished 5,000 updates in 1,380.10 seconds. Best validation loss was 2.3248 at update 2,400, compared with 2.4308 initially. All ten raw CPU replies are retained in `docs/performance/foundation-readout-evaluation.json`. They remain repetitive and incorrect: the model answered two plus three as 10.00 and replaced the supplied name Mira with an invented identity. This checkpoint is rejected for deployment. Cutting intercell paths still removes prompt information; that is insufficient evidence of useful language understanding.

`audit-checkpoint.py` compared every saved tensor with the pinned foundation. All frozen tensors, source wiring and cell IDs are unchanged. Of 26,542,080 allowed attention parameters, 26,542,001 differ from the original foundation. Total parameters remain 134,515,008. The complete audit is `docs/performance/foundation-readout-weights-audit.json`.
