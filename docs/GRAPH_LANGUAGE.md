...
The control completed 40,000 base updates and 10,000 assistant updates. Assistant validation loss reached 2.349, but simple replies still failed. The graph dialogue model's completed 64-conversation CPU report gives correct/mismatched/ablated losses 2.726/3.117/8.201 and repeated four-gram fraction 0.257; it remains rejected. See `performance/graph-language-dialogue-evaluation.json`.

A frozen-weight diagnostic excluded padded-slot keys from attention while preserving local self operations. It changed short-prompt replies but did not make either model useful. The runtime enables this only with an explicit `paddingMask` manifest flag; existing exports keep their original behavior. A behavioral test changes inactive cells' features by 100 and verifies that they cannot alter active readouts with the mask enabled. This is a verified isolation property, not a chat-quality fix. All plain/masked control probes and masked graph probes are in `performance/attention-control-and-padding-probes.json`. The evidence points to a broader training/capacity problem as well as graph-specific limitations; it does not isolate one cause.
