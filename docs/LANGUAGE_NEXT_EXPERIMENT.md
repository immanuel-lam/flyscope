...
The short-context follow-up also failed. Its best validation checkpoint improved held-out short-context loss to 2.836 and repetition fraction to 0.140, but broad loss worsened to 3.618. It answered a cat question with a claim about the capital of France. It remains undeployed. All short/broad outputs and simple probes are saved in `performance/language-short-candidate-evaluation.json`.

A persistent-prompt follow-up kept the mean prompt embedding as an external drive into input cells during every recurrent update. Its source edges and disjoint readout remained mandatory. It also failed: broad correct/mismatched/ablated losses were 3.705/3.917/8.508; short-context losses were 2.848/3.355/8.440. Short-context repeated four-gram fraction was 0.200. Raw replies still confused simple topics. This is an undeployed experiment, recorded in `performance/language-context-candidate-evaluation.json`. Graph-cut and prompt-pooling tests verify that future answer tokens cannot enter the pooled prompt and that output cells cannot receive the prompt without source connections.

The next architecture experiment is documented in [GRAPH_LANGUAGE.md](GRAPH_LANGUAGE.md). It uses ordered token slots and causal source-edge attention instead of the scalar recurrent bottleneck. Its existence and CPU parity do not establish useful language ability.
