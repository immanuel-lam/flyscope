...
After a reply, **Inspect model computation** opens an unrolled recurrent view. It shows the previous state and two updates for the first 8 input and first 16 readout cells in checkpoint order. These are the same 24 cells in all three columns, not separate layers. All 512 cells execute. All 82 source edges between this selected subset are shown; omitted cells still contribute. Color and opacity use actual signed state and edge contributions. Embeddings are injected at both updates; retention and bias terms also contribute. Output lines are learned readout weights, not source synapses. Next-token probabilities are raw full-vocabulary softmax before special-token generation masks. Values are captured during inference, not regenerated by an animation.

The step selector covers processed prompt and generated tokens. There is no automatic replay. Hover values expose source IDs, weights, contributions and input embeddings. Unit checks compare displayed snapshots with independent inference steps, and browser checks cover open, select and close behavior.
