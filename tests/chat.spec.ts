...
test('MaleCNS opens by default with working FlyGPT side chat without state replay',async({page})=>{
 ...
 await page.goto('/');
 await expect(page.getByRole('heading',{name:'Explore the fruit fly nervous system.',exact:true})).toBeVisible();
 ...
 await page.route('**/api/chat/generate', async route => { const response = await route.fetch(); await new Promise(resolve => setTimeout(resolve, 500)); await route.fulfill({ response }); });
 await page.getByRole('button',{name:'Send',exact:true}).click();
 await expect(page.getByRole('status').filter({hasText:'Thinking…'})).toBeVisible();
 await expect(page.locator('.chat-message.assistant')).toBeVisible({timeout:20000});
 ...
 await expect(page.locator('.brain-viewport canvas')).toHaveAttribute('data-activity-visible','false');
